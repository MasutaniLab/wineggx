# Windows版EGGX
Version 2.3  
2020年8月12日（Windows 10バージョン2004に対応）  
升谷 保博  
（大阪電気通信大学）  
...
- Version 2.3
  - Windows 10バージョン2004上ではキーボードのイベントを取得できなくなったので，PeekMessage()をウィンドウごとではなく，第2引数をNULLにして全体を調べるようにしました．
...
`ggetevent()`, `ggetxpress()`ではボタンの押下のみを検出している．大域変数`mousePressed`ではボタンが押されている状態を保持している（ボタンを離すまで`true`）．
