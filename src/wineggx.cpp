...
* @section Notes
*  wnで指定したウィンドゥのアプリケーション座標系を変更する．
*  ウィンドゥ上のウィンドゥ座標系(座標値は整数)は，左下が(0,0)で，右上が(xsize-1,ysize-1)であり，
*  デフォルトではアプリケーション座標系(座標値は実数)の座標値はウィンドゥ座標系のそれに一致している．
*  coordinate()関数を使う事により，アプリケーション座標系の(xa,ya)をウィンドゥ座標系の(xw,yw)に対応させ，
*  それぞれのスケーリングファクターをxscale，yscaleで指定する．すなわち，描画関数等における
*  アプリケーション座標(x, y)からウィンドゥ座標(x', y')への変換は，次の式により行なわれることを意味する．
*    x' = xw + (x−xa)*xscale
*    y' = yw + (y−ya)*yscale
*/
void coordinate(int wn, int xw, int yw, double xa, double ya, double xscale, double yscale)
{
  return gEggX.coordinate(wn, xw, yw, xa, ya, xscale, yscale);
}
